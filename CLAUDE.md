...
- **End-time label as text field** (v1.00.012) — Added `showEndTimeLabel` (parallel to `showEndTime`). Two checkboxes in the Display Modes panel: **Show End Time Label** (renders the end-time label line) and **Show End Time** (appends the computed clock time; implies label shown). Render: label shows when `showEndTimeLabel` true even if time suppressed → free text line; No Clock/Clock Only still hide it. Server `effLabel()` / client `effLabelClient()` enforce "time ⇒ label" and default legacy timers (no flag) to follow `showEndTime`, so existing timers are unchanged. New socket event `update-show-end-time-label`. Threaded through To Library dialog and new-timer dialog; grid save preserves the field (no dedicated grid column yet).
